...
stays **< 40 ms CPU on every sensor** — it reaches **mAP 0.721**. The **offline
...
|---|---|---|---:|---:|---:|---:|---:|
| **…EVK4_mag7.3** *(dim headline)* | EVK4 | g192_ctx **+ min-radius** *(→ ens+TTA offline)* | 0.858 | **0.924** | **0.890** | **0.891** | **0.896** |
...
| **Overall** | — | — | — | — | — | **mAP 0.721** | **mAP 0.715** |

Real-time overall: **precision 0.559, recall 0.849, F1 0.674** — **6010 TP**, 4744 FP,
1066 FN, a **10× gain** over the classical baseline. The decisive lever was a
...
deployed real-time system to **0.721**. Trained on rolf (RTX 2080 Ti).

> **Real-time vs offline (criterion #3).** Deployed **real-time = mAP 0.721**:
...
> 0.651→**0.677** (0.704→**0.719**); **min-radius** on the large EVK4 object then adds
> 0.881→**0.891** (→**0.721**). Report both: **0.721 real-time / 0.715 offline.**
...
1. The **dim EVK4 mag-7.3** sequence — the case the Technical Report singles out
   as hardest — is the strongest result: **AP 0.891 real-time / 0.896 offline,
   F1 0.890, recall 0.924**, above its typical-size oracle ceiling (0.714)
...
**DIoU+hinge size loss 0.719** → **EVK4 min-radius 0.721 (deployed real-time)**, a
**10× gain over the classical baseline**. Temporal context was the breakthrough for the dim-object
recall that ensembling/scale could not touch (Thuraya3 2×). The achievable
oracle ceiling is ~0.87 (§6.5b); the deployed system reaches 0.721 within the
40 ms CPU budget.
...
1. **The whole-test achievable ceiling is ≈ 0.87** (mean of each sequence's best
   strategy). We reach **0.721 real-time / 0.715 offline**; the remaining gap to
